...
@Query(value = "SELECT * from (SELECT wi.id, ci.user_name, ci.nick_name, ci.star_id, (SELECT    SUM(wi2.work_time)  "
		...
		+ "OR ci.wei_chat LIKE ?3)) wi2 ",nativeQuery = true)
List<WorkTimeBoard> getWorkTimeBoardCurMonth(Long laborUnionId, Long days, String searchCondition);
...
@Query(value = "SELECT * from (SELECT wi.id, ci.user_name, ci.nick_name, ci.star_id, (SELECT    SUM(wi2.work_time)  "
		...
		+ "AND (ci.user_name LIKE ?3   OR ci.nick_name LIKE ?3   OR ci.star_id LIKE ?3   OR ci.phone_number LIKE ?3   OR ci.qq LIKE ?3   "
		+ "OR ci.wei_chat LIKE ?3)) wi2",nativeQuery = true)
List<WorkTimeBoard> getWorkTimeBoardByDay(Long laborUnionId, List<String> days, String searchCondition);
}
